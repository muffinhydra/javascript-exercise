...
function prim(size) {
  ...
  while (frontier.length > 0) {
    ...
    // difference between 2 floor cells current random wall from frontier and current random floor in range
...
function findneighbour(field, center, range, type) {
  ...
  for (let index = 0; index < 4; index++) {
    try {
      if (field[center.x + range[index][0]][center.y + range[index][1]] == type) {
        temparray.push(newCoordinate(center.x + range[index][0], center.y + range[index][1]));
      }
    } catch {
      continue;
    }
